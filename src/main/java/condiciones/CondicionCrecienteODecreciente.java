...
public class CondicionCrecienteODecreciente extends Condicion {
	...
	public CondicionCrecienteODecreciente(CreceODecrece criterio, RegistroIndicador indicador, int ultimosAnios) {
	...
	public RankingEmpresa calcular(RankingEmpresa rEmpresa) throws ParseException {
		...
		for (int i = 0; i < periodos.size() - 1; i++) {
			...
			BigDecimal indicadorActual = this.aplicarIndicador(indicador, empresa.getNombre(),
					periodos.get(i).getAnio(), periodos.get(i).getSemestre());
			BigDecimal indicadorSiguiente = this.aplicarIndicador(indicador, empresa.getNombre(),
					periodos.get(i + 1).getAnio(), periodos.get(i + 1).getSemestre());
